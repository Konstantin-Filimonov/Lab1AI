"""
Этап 3. Автоматизация эксперимента.

Прогоняет весь набор запросов из prompts.py через модель, заданную в .env,
и добавляет результаты в results/experiment_results.csv (не перезаписывая
результаты других моделей — файл общий для всех прогонов).

Запуск (для каждой модели отдельно, после смены модели в LM Studio и .env):
